...
//  - 'cepat'          → ready_in_minutes <= 30
//  - 'hemat'          → price_idr <= 30000
...
export function recipeMatchesDiet(recipe: Record<string, unknown>, slug: string): boolean {
  ...
  if (slug === "cepat") return Number(recipe.ready_in_minutes) <= 30;
  if (slug === "hemat") return Number(recipe.price_idr) <= 30000;
