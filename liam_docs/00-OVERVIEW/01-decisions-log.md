...
## ADR-014 — Paket "Belanja di Kami": Tabel `packages` + Harga Agregasi

**Tanggal:** 2026-06-14
**Status:** Accepted

**Konteks:** Notulen tim minta fitur belanja dipisah 2 tab: "Belanja Sendiri"
(menu hasil generate AI — bahan belum tentu kami stok) vs "Belanja di Kami" (paket
menu fiks yang bahannya kami sediakan, hanya ~3 paket). Notulen #4 menegaskan
"FoodPlan & Prep tidak masuk generate; paket = periode + menu fiks, porsi bisa
request".

> Catatan penomoran: ADR-013 dipakai branch `feat/regenerate-day` (PR #26, belum
> merge saat ADR ini ditulis). ADR paket sengaja diberi nomor 014 agar tidak bentrok.

**Keputusan:**
1. **Tabel baru `packages` + `package_meals`** (bukan flag di `recipes`).
   Menangkap konsep "periode + menu fiks". `package_meals` refer ke `recipes`
   katalog → bahan & harga ikut `recipe_ingredients` (single source of truth).
2. **Harga paket = agregasi `recipe_ingredients`** (skala per porsi yang
   di-request), BUKAN kolom statis. `packages.price_idr` cuma harga tampilan opsional.
3. **2 tab di halaman Belanja (`/shopping`)**, bukan route `/packages` baru —
   menjaga bottom-nav tetap ≤5 item & setia ke notulen #5.
4. **Order "Belanja di Kami" reuse `orderService.createOrder`** (planId=null,
   outputType='package') → WhatsApp. Tanpa route/tabel order baru.
5. **`saved_shopping_lists`** (tabel baru, owner-only RLS) untuk fitur simpan
   daftar belanja (notulen #13), snapshot `items_json`.
6. **Util `buildShoppingList.js` bersama** dipakai tab planner & paket — satu
   logika agregasi (port dari logika lama ShoppingList + shoppingList.ts Edge).

**Alasan:**
- Refer ke recipes = tidak duplikat data bahan, "tambah bahan di Supabase" (#2)
  cukup lengkapi `recipe_ingredients`.
- Harga agregasi = konsisten antara daftar belanja & total order, tidak ada angka
  "ajaib" yang lepas dari bahan.
- Reuse orderService & util = permukaan kode minimal, konsisten dgn fitur order lama.

**Konsekuensi:**
- Paket dibatasi ke resep yang punya `recipe_ingredients` + `price_idr` lengkap
  (saat seed: resep 1-6). Resep tanpa harga bahan tidak cocok jadi menu paket.
- Belum ada admin UI kelola paket — seed manual via migrasi dulu (fase lanjutan).

**Alternatif ditolak:**
- Flag `is_supplied` di `recipes` → tidak menangkap "paket = periode+menu fiks".
- Tabel stok bahan terpisah (pemilahan per-bahan) → kompleks, belum perlu untuk 3 paket.
- Route `/packages` terpisah → nambah nav item (>5) atau halaman yatim.
